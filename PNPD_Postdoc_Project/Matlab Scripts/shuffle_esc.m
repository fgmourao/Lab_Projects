function shf_sig = shuffle_esc(sig, Fs,numshf)
...
% numshf(OPCIONAL, MODIFICADO): number of shuffled segments. default = min(1000,signalLength(seconds))
%
% OUTPUTS:
...
%UPDATE 21/01/2015 VINÍCIUS
if size(sig,1) == 1 %corrects if sig is not collumn vector
	sig = sig';
end
%END UPDATE
